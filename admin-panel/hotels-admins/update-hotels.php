...
if(isset($_GET['id'])){
  ...
  $hotel = $conn->query("SELECT * FROM resort_accom WHERE id='$id'");
  $hotel->execute();
  ...
  if(isset($_POST['submit'])){
    if(empty($_POST['name']) OR empty($_POST['description'])){
      ...
      $update = $conn->prepare("UPDATE resort_accom SET name = :name, description = :description WHERE id='$id'");
      $update->execute([
        ":name"=> $name,
...
?>
       ...
       <div class="row">
        <div class="col">
          <div class="card">
            <div class="card-body">
              <h5 class="card-title mb-5 d-inline">Update Resort Accommodation</h5>
